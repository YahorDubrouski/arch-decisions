...
export interface ValidationSuccess { success: true; context: ProjectContext; }
export interface ValidationFailure { success: false; errors: string[]; }
export type ValidationResult = ValidationSuccess | ValidationFailure;

export function validateEvaluateDecisionsRequest(body: unknown): ValidationResult {
  if (!validateContext(body)) {
    return {success: false, errors: getValidationErrors(body)};
  }
...
function getValidationErrors(value: unknown): string[] {
  ...
  if (ctx.trafficPattern === undefined || !['low-steady', 'variable', 'high-spike', 'unpredictable'].includes(String(ctx.trafficPattern))) {
    errors.push('trafficPattern is required and must be one of: low-steady, variable, high-spike, unpredictable');
  }
  if (ctx.budgetSensitivity === undefined || !['cost-optimized', 'balanced', 'performance-first'].includes(String(ctx.budgetSensitivity))) {
  ...
  if (ctx.operationalMaturity === undefined || !['minimal', 'moderate', 'advanced', 'enterprise'].includes(String(ctx.operationalMaturity))) {
